...
A busca sistemática pela maximização do fluxo em uma rede requer uma estrutura auxiliar que quantifique não apenas a capacidade ociosa das conexões, mas também a possibilidade de reversão de decisões de roteamento subótimas estabelecidas em iterações prévias. Para este fim, introduz-se o conceito de digrafo residual (ou rede residual) $D_f = (V, E_f)$~\cite{ahuja1993network, ford1956maximal}.
...
Suponha que $f$ seja um fluxo máximo. Por definição, não existe um caminho aumentante de $s$ para $t$ na rede residual $D_f$~\cite{ahuja1993network}. Definimos o conjunto $S$ como o conjunto de todos os vértices que são alcançáveis a partir da fonte $s$ em $D_f$ através de arcos com capacidade residual estritamente positiva ($c_f > 0$). Definimos $T = V \setminus S$ como o restante dos vértices.
...
A resolução computacional do problema de fluxo máximo fundamenta-se historicamente no método iterativo proposto por Ford e Fulkerson~\cite{ford1956maximal}. A premissa estrutural deste paradigma consiste na manutenção de uma rede residual $D_f$ e na busca sistemática por caminhos aumentantes — percursos simples direcionados da fonte $s$ ao sumidouro $t$ onde todas as arcos possuem capacidade residual estritamente positiva.
...
A evolução central desta arquitetura baseia-se na separação rigorosa entre a descoberta de caminhos e o roteamento de fluxo. No início de cada fase iterativa, uma Busca em Largura (BFS) global é executada a partir da fonte $s$ para atribuir níveis topológicos $d(v)$ a todos os vértices, correspondentes às suas distâncias mínimas exatas na rede residual $D_f$. O Digrafo de Níveis gerado é, por definição, um subdigrafo direcionado acíclico (DAG) composto estritamente pelas arcos residuais que avançam exatamente um nível em direção ao sumidouro, satisfazendo a restrição geométrica $d(v) = d(u) + 1$~\cite{ahuja1993network}.
...
\begin{itemize}
    \item \textbf{Operação de Empurrar (\textit{Push}):} Esta ação é executada se, e somente se, o vértice ativo $u$ possuir uma arco residual $(u,v)$ estritamente admissível. Uma arco é considerada admissível quando apresenta capacidade residual não-nula ($c_f(u,v) > 0$) e um declive topológico unitário perfeito em direção ao destino ($h(u) = h(v) + 1$). A quantidade de fluxo transferida é algebricamente delimitada por $\delta = \min(e(u), c_f(u,v))$. Caso o empurrão transfira exatamente $c_f(u,v)$, a arco é plenamente saturada e removida temporariamente de $D_f$; caso contrário, trata-se de um empurrão não-saturante, cujo efeito primário e imediato é zerar o excesso do vértice $u$~\cite{cook1998combinatorial}.
...
Embora existam algoritmos específicos para este fim, o problema pode ser elegantemente reduzido ao cômputo de um fluxo máximo~\cite{ahuja1993network}. A redução algorítmica procede através da construção de uma rede de fluxo direcionada $D' = (V', E')$ baseada no digrafo original $D$:
\begin{enumerate}
...
Consequentemente, a restrição de capacidade unitária nas arcos conectadas à super-fonte e ao super-sumidouro força a lei de conservação a atuar como um restritor de exclusividade: se $1$ unidade de fluxo transita por um vértice $u \in L$, ela só pode ser roteada por uma única arco em direção a $R$. O valor numérico do fluxo máximo computado pela rede $D'$ equivale perfeitamente à cardinalidade $|M|$ do emparelhamento máximo, e as arcos ativas ($f(u,v) = 1$) que cruzam de $L$ para $R$ constituem os pares da solução exata.
...
A extração computacional dos vértices que compõem a cobertura mínima exige uma análise minuciosa da rede residual $D_f$ gerada logo após a convergência do fluxo máximo. A biblioteca desenvolvida neste projeto recupera o conjunto $C$ através do seguinte procedimento algorítmico baseado em cortes mínimos:

\begin{enumerate}
    \item Após computar o emparelhamento máximo, executa-se uma Busca em Largura (BFS) ou Busca em Profundidade (DFS) partindo da super-fonte $s$ na rede residual $D_f$, transitando exclusivamente por arcos que possuam capacidade residual estritamente positiva ($c_f(u,v) > 0$).
...
A técnica de \textit{Vertex Splitting} altera a topologia do digrafo original $D$ para gerar uma nova rede de fluxo estendida $D'$ através da seguinte heurística de mapeamento:
\begin{enumerate}
    ...
    \item Todas as arcos que originalmente apontavam para $v$ no digrafo $D$ são redirecionadas para apontar exclusivamente para $v_{in}$.
    \item Todas as arcos que originalmente divergiam de $v$ no digrafo $D$ passam a emergir exclusivamente a partir de $v_{out}$ (com capacidade 1 ou infinita, dependendo da formulação).
\end{enumerate}

Esta intervenção estrutural é genial em sua simplicidade: ao forçar todo o fluxo que transita pelo nó original $v$ a atravessar um "gargalo" artificial $(v_{in}, v_{out})$ de capacidade $1$, a rede impõe fisicamente que não mais do que $1$ unidade de fluxo possa cruzar aquele vértice geométrico. Ao submeter esta nova topologia estendida $D'$ à arquitetura orientada a objetos (Dinic ou Push-Relabel), o fluxo máximo computado atestará rigorosamente o número máximo de rotas disjuntas por vértices, resolvendo o problema de conectividade nodal sem exigir nenhuma alteração no código do algoritmo subjacente~\cite{korte2018combinatorial}.
